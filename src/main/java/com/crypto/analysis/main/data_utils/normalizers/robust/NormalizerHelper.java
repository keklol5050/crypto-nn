...
public class NormalizerHelper {
    ...
    public static void loadData(Coin coin) {
        ...
        List<String> fundList = switch (coin) {
            case BTCUSDT ->
                    Files.readAllLines(pathToBTCFunding);
            default -> throw new IllegalArgumentException();
        };
        List<String> metrics = switch (coin) {
            case BTCUSDT ->
                    Files.readAllLines(pathToBTCMetrics);
            default -> throw new IllegalArgumentException();
        };
        ...
        for (Map.Entry<Date, double[]> entry : sentiment.getMap().entrySet()) {
            if (entry.getKey().toInstant().atZone(ZoneId.systemDefault()).toLocalDate().isBefore(START_DATE)) continue;
